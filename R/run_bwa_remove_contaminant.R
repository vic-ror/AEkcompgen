...
#' @param output FASTQ file with the reads that mapped in the contaminant reference genome removed.
...
#'  ref = temp_ref_fasta,
#'  keep_index = FALSE)
#' #3. Unlink output file
#' unlink("test_mapped.fastq.gz")
#'
run_bwa_remove_contaminant <- function(input,
  ...
  if(length(input) == 1){
    ...
    system(glue::glue("samtools fastq -o {output}.fastq.gz {input}_removed_cont_sorted.bam"))
  }
    ...
    message("Creating fastaq file from with reads that were not mapped in contaminant...")
    system(glue::glue("samtools fastq -1 {output[1]}.fastq.gz -2 {output[2]}.fastq.gz {input[1]}_removed_cont_sorted.bam"))
