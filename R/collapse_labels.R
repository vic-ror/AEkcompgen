#' Joins different labels in a single line, based on the cluster they belong to.
#'
...
#'
#' @return A dataframe containing the collumns "cluster" and "labels".
#' @export
...
#' collapsed_file <- collapse_labels(cdhit_out)
#' # 3. View result
#' print(collapsed_file)

collapse_labels <- function(dataframe_cd, path_db = NULL){
  ...
  return(cluster_label_joined_reordered)
}
