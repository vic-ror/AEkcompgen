#' Selects the clusters that contain more than one dataset label
#'
...
#' shared_clusters <- select_shared_clusters(cdhit_out)
#' # 3. View result
#' print(shared_clusters)
select_shared_clusters <- function(dataframe_cd, path_db = NULL){
  ...
  multiple_labels <- input_id_label |>
  ...
  #Recover the information from the label exclusive clusters
  multiple_label_info <- dplyr::inner_join(input_id_label,
                                          multiple_labels,
                                          by = "cluster") |>
  ...
  shared_clusters_quantity <- multiple_label_info |>
